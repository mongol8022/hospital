...
//            mysqli_select_db($db, $config["database"]["name"]) or die("Error connecting to db."); 
 ...
 //               echo '</option>';
        ...
        case 'add':
                if (isset($_GET["time_begin"]) and isset($_GET["count"]) and isset($_GET["valid_from"]) and isset($_GET["service_id"]) and isset($_GET["week_day"]) and isset($_GET["worplace_id"]) and isset($_GET["valid_to"]))
                {
                 if ($_GET["valid_to"] === 'null') { $valid_to = 'NULL';}
                 else {$valid_to = date("Y-m-d", strtotime($_GET["valid_to"]));}
                    $result = mysqli_query($db, "INSERT INTO schedule (time_begin, count, valid_from, valid_to, service_id, worplace_id, week_day) VALUES ('".$_GET["time_begin"].":00', '".$_GET["count"]."', '".date("Y-m-d", strtotime($_GET["valid_from"]))."', ".$valid_to.", '".$_GET["service_id"]."', '".$_GET["worplace_id"]."', '".$_GET["week_day"]."')"); 
                    echo $result;
                }
                else
                {
        ...
        case 'edit':
                if (isset($_GET["time_begin"]) and isset($_GET["count"]) and isset($_GET["valid_from"]) and isset($_GET["service_id"]) and isset($_GET["week_day"]) and isset($_GET["worplace_id"]) and isset($_GET["valid_to"]) and isset($_GET["id"]))
                {
                    ...
                    $result = mysqli_query($db, "update schedule SET time_begin = '".$_GET["time_begin"].":00', count = '".$_GET["count"]."', valid_from = '".date("Y-m-d", strtotime($_GET["valid_from"]))."', valid_to = ".$valid_to.", service_id = '".$_GET["service_id"]."', worplace_id = '".$_GET["worplace_id"]."', week_day = '".$_GET["week_day"]."' WHERE id = ".$_GET["id"]); 
                    echo $result;
                }
                else
                {
